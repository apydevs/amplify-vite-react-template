...
import { SelectBoxRadiusProps} from "../interfaces/interfaces.tsx";
...
const SelectBoxValue: React.FC<SelectBoxRadiusProps> = ({ onChange, name }) => {
    const [selected, setSelected] = useState<ValuationType>({ id: 1, "value": 0,text: "Any" })
    const selectedFilters = useSelector((state: RootState) => state.filters);
    useEffect(() => {
        console.log(`${name}Valuation`)
        const fetchData = async () => {
            const index = data.findIndex(item => item.value === selectedFilters[`${name}Valuation` as keyof typeof selectedFilters]);
    ...
    const handleChange = (item: ValuationType) => {
        setSelected(item);
        onChange(item);
        console.log(item)
    }
