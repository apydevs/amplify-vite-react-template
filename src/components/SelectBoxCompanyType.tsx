// src/components/SelectBoxCompanyType.tsx
import { useState } from 'react';
import { Listbox, ListboxButton, ListboxOption, ListboxOptions } from '@headlessui/react';
import { CheckIcon, ChevronUpDownIcon } from '@heroicons/react/20/solid';

// Define the type for company type options
...
interface SelectBoxCompanyTypeProps {
    onSelectedChange: (selected: CompanyType) => void;
}
...
export default function SelectBoxCompanyType({ onSelectedChange }: SelectBoxCompanyTypeProps) {
    const [selected, setSelected] = useState<CompanyType>(data[1]);

    const handleChange = (selectedValue: CompanyType) => {
        setSelected(selectedValue);
        onSelectedChange(selectedValue); // Call parent callback with selected value
    };
            ...
            <div className="relative">
                ...
                <ListboxOptions className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                    {data.map((companyType) => (
                        ...
                        >
                            <span className="block truncate font-normal group-data-[selected]:font-semibold">{companyType.name}</span>
