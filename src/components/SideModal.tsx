...
import {useOfferPacks} from "../hooks/useOfferPacks.ts";
                                ...
                                <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                                    <div className="px-4 py-6 sm:px-6 border-b">
                                        <div className="flex items-start justify-between">
                                            <DialogTitle className="text-base font-semibold leading-6 text-gray-900">Offer
                                                Packs Available</DialogTitle>
                                    ...
                                    {/* Main */}
                                    <div
                                        className="divide-x divide-gray-200 flex flex-col lg:flex-row mx-auto container">
                                        <div className="w-full xl:w-1/2 h-screen overflow-y-scroll">
                                            <h2 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl px-10 pt-5">Available
                                                Offer Packs</h2>

                                            {packs.map((item) =>

                                                <PackCard id={item.id} price={item.price} title={item.title}
                                                          offer={item.offer} exclusive={item.exclusive}
                                                          selected={item.selected} onSelect={handlePackSelection}/>
                                            )}
                                            ...
                                            <div className="mx-auto max-w-2xl sm:text-center mt-5 px-10 lg:px-2.5">
                                                ...
                                                {selectedPack ? (
                                                    <p className="mt-6 text-lg leading-8 text-gray-600">
                                                        You have selected an offer pack that
                                                        contains {pluralize(selectedPack.offer, 'offer')}.
