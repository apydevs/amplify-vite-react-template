...
export default function CallToAction1(
    {
        title = 'Default Title',
        textContent = 'default-slug',
       action = 'default-layout',
    }: Layout3Props) {

    const text = { __html: textContent };
    //const action = { __html: action };
    ...
    return (
        ...
        <div className="relative isolate overflow-hidden bg-gray-700 px-6 py-24 text-center shadow-2xl sm:rounded-3xl sm:px-16">
            ...
            <p className="mx-auto mt-6 max-w-4xl px-10 text-lg leading-8 text-gray-300 pb-5" dangerouslySetInnerHTML={text}>

            </p> {action && (
            <Link title={'join us today'} to={action} className="text-white px-3 py-2.5 hover:text-green-300 hover:animate-pulse underline underline-offset-2 text-2xl font-bold ">Join us today!</Link>
        )}
